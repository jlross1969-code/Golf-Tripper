...
export interface SideMatchResultsData {
  tripName: string;
  roundName: string;
  roundDate: string;
  matches: { type: string; groupName: string; status: string; leader?: string | null; rows: { name: string; result: string }[] }[];
}

export async function generateSideMatchResultsPDF(data: SideMatchResultsData): Promise<Buffer> {
  const doc = new PDFDocument({ margin: 40, size: "A4" });
  const contentW = doc.page.width - 80;
  doc.rect(0, 0, doc.page.width, 70).fill(GREEN);
  doc.fillColor(WHITE).fontSize(20).font("Helvetica-Bold").text(data.tripName, 40, 18);
  doc.fontSize(12).font("Helvetica").text(`${data.roundName}  •  Side-match results  •  ${data.roundDate}`, 40, 44);
  let y = 92;
  if (!data.matches.length) {
    doc.fillColor(MID_GREY).fontSize(12).font("Helvetica").text("No additional side matches were recorded for this round.", 40, y);
  }
  for (const match of data.matches) {
    if (y > doc.page.height - 130) { doc.addPage(); y = 40; }
    doc.rect(40, y, contentW, 24).fill(LIGHT_GREEN);
    doc.fillColor(GREEN).fontSize(11).font("Helvetica-Bold").text(`${match.type} — ${match.groupName}`, 48, y + 6);
    doc.fillColor(MID_GREY).fontSize(9).font("Helvetica").text(`${match.status}${match.leader ? `  •  Leading: ${match.leader}` : ""}`, 48, y + 17);
    y += 34;
    doc.rect(40, y, contentW, 18).fill(DARK_GREY);
    doc.fillColor(WHITE).fontSize(9).font("Helvetica-Bold").text("Player", 48, y + 4).text("Live result", 330, y + 4, { width: 210, align: "right" });
    y += 18;
    for (const [index, row] of match.rows.entries()) {
      doc.rect(40, y, contentW, 18).fill(index % 2 === 0 ? WHITE : LIGHT_GREY);
      doc.fillColor(DARK_GREY).fontSize(9).font("Helvetica").text(`${index + 1}. ${row.name}`, 48, y + 4).text(row.result, 330, y + 4, { width: 210, align: "right" });
      y += 18;
    }
    y += 12;
  }
  doc.fillColor(MID_GREY).fontSize(8).font("Helvetica").text(`Generated by Golf Trip App  •  ${new Date().toLocaleDateString()}`, 40, doc.page.height - 30, { align: "center", width: contentW });
  return bufferFromDoc(doc);
}
...
export interface ItineraryPdfData {
  ...
  startDate: string;
  endDate: string;
  items: { type: string; title: string; location?: string | null; startsAt?: string | null; endsAt?: string | null; notes?: string | null; players: string[] }[];
}
...
export async function generateItineraryPDF(data: ItineraryPdfData): Promise<Buffer> {
  ...
  doc.rect(0, 0, doc.page.width, 70).fill(GREEN);
  doc.fillColor(WHITE).fontSize(20).font("Helvetica-Bold").text(data.tripName, 40, 18);
  doc.fontSize(12).font("Helvetica").text(`Travel itinerary  •  ${data.startDate} – ${data.endDate}`, 40, 44);
  let y = 92;
  if (!data.items.length) doc.fillColor(MID_GREY).fontSize(12).font("Helvetica").text("No itinerary items have been added yet.", 40, y);
  for (const item of data.items) {
    if (y > doc.page.height - 150) { doc.addPage(); y = 40; }
    doc.rect(40, y, contentW, 22).fill(LIGHT_GREEN);
    doc.fillColor(GREEN).fontSize(11).font("Helvetica-Bold").text(`${item.type.toUpperCase()}  •  ${item.title}`, 48, y + 6);
    y += 28;
    const details = [item.location, item.startsAt ? `Starts: ${item.startsAt}` : null, item.endsAt ? `Ends: ${item.endsAt}` : null, item.players.length ? `Assigned: ${item.players.join(", ")}` : null, item.notes].filter(Boolean) as string[];
    for (const detail of details) {
      doc.fillColor(DARK_GREY).fontSize(9).font("Helvetica").text(detail, 48, y, { width: contentW - 16 });
      y += Math.max(14, doc.heightOfString(detail, { width: contentW - 16 }) + 3);
    }
    y += 10;
  }
  doc.fillColor(MID_GREY).fontSize(8).font("Helvetica").text(`Generated by Golf Trip App  •  ${new Date().toLocaleDateString()}`, 40, doc.page.height - 30, { align: "center", width: contentW });
...
export async function generateExpenseCategoryReportPDF(data: { tripName: string; categories: { category: string; amountCents: number }[]; totalCents: number; generatedAt: string }): Promise<Buffer> {
  const doc = new PDFDocument({ margin: 40, size: "A4" });
  const contentW = doc.page.width - 80;
  doc.rect(0, 0, doc.page.width, 70).fill(GREEN);
  doc.fillColor(WHITE).fontSize(20).font("Helvetica-Bold").text(data.tripName, 40, 18);
  doc.fontSize(12).font("Helvetica").text("Approved expense category report", 40, 44);
  let y = 96;
  doc.rect(40, y, contentW, 20).fill(DARK_GREY);
  doc.fillColor(WHITE).fontSize(10).font("Helvetica-Bold").text("Category", 48, y + 5).text("Approved spend", 340, y + 5, { width: 200, align: "right" });
  y += 20;
  if (!data.categories.length) doc.fillColor(MID_GREY).fontSize(11).font("Helvetica").text("No approved actual expenses have been recorded.", 48, y + 12);
  for (const [index, entry] of data.categories.entries()) {
    doc.rect(40, y, contentW, 22).fill(index % 2 === 0 ? WHITE : LIGHT_GREY);
    doc.fillColor(DARK_GREY).fontSize(10).font("Helvetica").text(entry.category, 48, y + 6).text(`$${(entry.amountCents / 100).toFixed(2)}`, 340, y + 6, { width: 200, align: "right" });
    y += 22;
  }
  y += 8;
  doc.rect(40, y, contentW, 24).fill(GREEN);
  doc.fillColor(WHITE).fontSize(11).font("Helvetica-Bold").text("TOTAL APPROVED ACTUAL SPEND", 48, y + 6).text(`$${(data.totalCents / 100).toFixed(2)}`, 340, y + 6, { width: 200, align: "right" });
  doc.fillColor(MID_GREY).fontSize(8).font("Helvetica").text(`Generated by Golf Trip App  •  ${data.generatedAt}`, 40, doc.page.height - 30, { align: "center", width: contentW });
  return bufferFromDoc(doc);
}

export async function generateTripFinanceSummaryPDF(data: { tripName: string; generatedAt: string; budgetCents: number; approvedActualCents: number; pendingExpenseCount: number; varianceCents: number; outstandingPlayerCents: number; outstandingSupplierCents: number; categories: { category: string; amountCents: number }[] }): Promise<Buffer> {
  const doc = new PDFDocument({ margin: 40, size: "A4" });
  const contentW = doc.page.width - 80;
  doc.rect(0, 0, doc.page.width, 70).fill(GREEN);
  doc.fillColor(WHITE).fontSize(20).font("Helvetica-Bold").text(data.tripName, 40, 18);
  doc.fontSize(12).font("Helvetica").text("Trip finance summary", 40, 44);
  const cards = [["Planned budget", data.budgetCents], ["Approved actual", data.approvedActualCents], ["Variance", data.varianceCents], ["Player outstanding", data.outstandingPlayerCents], ["Supplier outstanding", data.outstandingSupplierCents]];
  let y = 96;
  for (const [label, value] of cards) {
    doc.rect(40, y, contentW, 30).fill(LIGHT_GREEN);
    doc.fillColor(GREEN).fontSize(11).font("Helvetica-Bold").text(String(label), 50, y + 9);
    doc.text(`$${(Number(value) / 100).toFixed(2)}`, 340, y + 9, { width: 190, align: "right" });
    y += 34;
  }
  doc.fillColor(MID_GREY).fontSize(10).font("Helvetica").text(`Pending expense approvals: ${data.pendingExpenseCount}`, 40, y + 6);
  y += 32;
  doc.rect(40, y, contentW, 20).fill(DARK_GREY);
  doc.fillColor(WHITE).fontSize(10).font("Helvetica-Bold").text("Approved spend by category", 48, y + 5).text("Amount", 340, y + 5, { width: 190, align: "right" });
  y += 20;
  if (!data.categories.length) doc.fillColor(MID_GREY).fontSize(10).font("Helvetica").text("No approved actual expenses have been recorded.", 48, y + 8);
  for (const [index, entry] of data.categories.entries()) {
    doc.rect(40, y, contentW, 22).fill(index % 2 === 0 ? WHITE : LIGHT_GREY);
    doc.fillColor(DARK_GREY).fontSize(10).font("Helvetica").text(entry.category, 48, y + 6).text(`$${(entry.amountCents / 100).toFixed(2)}`, 340, y + 6, { width: 190, align: "right" });
    y += 22;
  }
  doc.fillColor(MID_GREY).fontSize(8).font("Helvetica").text(`Generated by Golf Trip App  •  ${data.generatedAt}`, 40, doc.page.height - 30, { align: "center", width: contentW });
  return bufferFromDoc(doc);
