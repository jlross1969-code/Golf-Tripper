  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-4rem)] max-w-2xl flex-col pb-28 sm:pb-0">
      <div className="flex items-center gap-3 border-b border-border bg-card px-4 py-3">
        ...
        {moderationStatus?.canModerate && <div className="flex gap-1"><Button variant="outline" size="sm" onClick={() => setAnalyticsOpen(true)}>Activity</Button><Button variant="outline" size="sm" onClick={() => setReportsOpen(true)} className="gap-1.5"><ShieldAlert className="h-4 w-4" />Reports</Button></div>}
      </div>
      ...
      <Dialog open={analyticsOpen} onOpenChange={setAnalyticsOpen}><DialogContent><DialogHeader><DialogTitle>Photo activity</DialogTitle><DialogDescription>Anonymous trip-level totals only. Individual player activity is not collected or shown.</DialogDescription></DialogHeader><div className="grid grid-cols-3 gap-3"><div className="rounded-xl border border-border p-3 text-center"><p className="text-lg font-bold">{photoActionSummary?.downloads ?? 0}</p><p className="text-xs text-muted-foreground">Downloads</p></div><div className="rounded-xl border border-border p-3 text-center"><p className="text-lg font-bold">{photoActionSummary?.shares ?? 0}</p><p className="text-xs text-muted-foreground">Shares</p></div><div className="rounded-xl border border-border p-3 text-center"><p className="text-lg font-bold">{photoActionSummary?.total ?? 0}</p><p className="text-xs text-muted-foreground">Total actions</p></div></div></DialogContent></Dialog>
