...
export default function AdminFinancialReports() {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="mx-auto max-w-3xl space-y-4 px-5 py-7">
      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Download className="h-4 w-4 text-primary" />Approved expense category report</CardTitle></CardHeader><CardContent><p className="mb-3 text-sm text-muted-foreground">Download approved actual spending grouped by category or the full supplier invoice review audit.</p><div className="flex flex-wrap gap-2"><a href={`/api/export/expense-categories/${id}`}><Button variant="outline">Categories CSV</Button></a><a href={`/api/pdf/expense-categories/${id}`} target="_blank" rel="noreferrer"><Button>Categories PDF</Button></a><a href={`/api/export/supplier-invoice-audit/${id}`}><Button variant="outline">Invoice audit CSV</Button></a></div></CardContent></Card>
